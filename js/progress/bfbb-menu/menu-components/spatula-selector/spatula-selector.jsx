...
export default class SpatulaSelector extends React.Component {
  ...
  upJellyfishClick = () => {
    this.setState(
      (state) => ({
        selectedArea:
          state.selectedArea > 0 ? state.selectedArea - 1 : spatulas.length - 1,
        selectedSpatula: 0,
      }),
      () => {
  ...
  downJellyfishClick = () => {
    this.setState(
      (state) => ({
        selectedArea:
          state.selectedArea < spatulas.length - 1 ? state.selectedArea + 1 : 0,
        selectedSpatula: 0,
      }),
      () => {
        move_sound.currentTime = 0;
      ...
      <>
          ...
          <div className="inner-selector">
            ...
            {spatulas.map((area, index) => (
              <div
                ...
                id={
                  this.state.selectedArea === index
                    ? "area-bubble-selected"
                    : ""
                }
                key={index}
                onClick={() => {
                  this.onAreaBubbleClick(index);
                }}
              ></div>
            ))}
            <div
        ...
        <div className="spatula-selector">
          ...
          <div className="spatula-container">
            {spatulas[this.state.selectedArea].spatulas.map(
              (spatula, index) => (
                <div
                  key={index}
                  onMouseOver={() => {
                    this.onSpatulaHover(index);
                  }}
                  onClick={() => {
                    this.onSpatulaClick();
                  }}
                >
                  <Spatula
                    key={index}
                    gold={spatulaCount - totalPreviousSpatulas > index}
                    selected={index === this.state.selectedSpatula}
                  ></Spatula>
                </div>
              )
            )}
          ...
          <p id="spatula-name">
            {
              spatulas[this.state.selectedArea].spatulas[
                this.state.selectedSpatula
              ]
            }
          </p>
        </div>
